...
/**
 * @brief 串口错误回调函数 (防止串口锁死)
 */
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    // 1. 获取对应的 UART 管理对象
    Struct_UART_Manage_Object *pManageObject = NULL;

    if (huart->Instance == USART1) pManageObject = &UART1_Manage_Object; // 裁判系统
    else if (huart->Instance == USART2) pManageObject = &UART2_Manage_Object;
    else if (huart->Instance == USART3) pManageObject = &UART3_Manage_Object; // 遥控器?
    else if (huart->Instance == UART4)  pManageObject = &UART4_Manage_Object;
    else if (huart->Instance == UART5)  pManageObject = &UART5_Manage_Object;
    else if (huart->Instance == USART6) pManageObject = &UART6_Manage_Object;

    // 2. 如果找到了对应的对象，进行复活操作
    if (pManageObject != NULL)
    {
        // 步骤A: 清除常见的硬件错误标志 (ORE, NE, FE, PE)
        // 关键是 ORE (Overrun)，不清除它就无法重新启动接收
        __HAL_UART_CLEAR_OREFLAG(huart);
        __HAL_UART_CLEAR_NEFLAG(huart);
        __HAL_UART_CLEAR_FEFLAG(huart);
        __HAL_UART_CLEAR_PEFLAG(huart);

        // 步骤B: 重新开启 DMA 空闲中断接收 (复活串口)
        HAL_UARTEx_ReceiveToIdle_DMA(huart, pManageObject->Rx_Buffer, pManageObject->Rx_Buffer_Length);
