...
void Class_FSM_Heat_Detect::Reload_TIM_Status_PeriodElapsedCallback()
{
    ...
    case (3):
    {
        ...
        if (abs(Booster->Fric[1].Get_Now_Omega_Radian()) >= Booster->Friction_Omega_Threshold)
        {
            //开机了->正常状态
            Set_Status(0);
        }
...
void Class_Booster::Output()
{
    ...
    switch (Booster_Control_Type)
    {
        case (Booster_Control_Type_DISABLE):
        {
            ...
            Fric[0].Set_DJI_Motor_Control_Method(DJI_Motor_Control_Method_OMEGA);
             Fric[1].Set_DJI_Motor_Control_Method(DJI_Motor_Control_Method_OMEGA);
              Fric[2].Set_DJI_Motor_Control_Method(DJI_Motor_Control_Method_OMEGA);
               Fric[3].Set_DJI_Motor_Control_Method(DJI_Motor_Control_Method_OMEGA);
            ...
            Fric[0].PID_Angle.Set_Integral_Error(0.0f);
            Fric[1].PID_Angle.Set_Integral_Error(0.0f);
            Fric[2].PID_Angle.Set_Integral_Error(0.0f);
            Fric[3].PID_Angle.Set_Integral_Error(0.0f);
    ...
    //  控制摩擦轮
    Friction_Control_Type = Enum_Friction_Control_Type(Flag_flag1);
    if(Friction_Control_Type != Friction_Control_Type_DISABLE)
    {
        Fric[0].Set_Target_Omega_Rpm(Fric_Low_Rpm);//Fric_Low_Rpm
        Fric[1].Set_Target_Omega_Rpm(-Fric_Low_Rpm);
        Fric[2].Set_Target_Omega_Rpm(Fric_High_Rpm);//Fric_High_Rpm
        Fric[3].Set_Target_Omega_Rpm(-Fric_High_Rpm);
    }
    else
    {
        Fric[0].Set_Target_Omega_Rpm(0.0f);
        Fric[1].Set_Target_Omega_Rpm(0.0f);
        Fric[2].Set_Target_Omega_Rpm(0.0f);
        Fric[3].Set_Target_Omega_Rpm(0.0f);
...
{     
    ...
    Fric[0].TIM_PID_PeriodElapsedCallback();
