...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	printf("PID: Enabled for speed control\r\n");  // 添加PID控制器状态信息
	printf("Ready!\r\n\r\n");
  ...
  while (1)
  {
		...
		{
			// 在上位机控制模式下，可以使用PID速度控制
			// 这里可以添加更复杂的控制逻辑
			// 例如：Speed_Control_Update(100.0f, 100.0f); // 设置左右轮目标速度为100mm/s
			
			// 如果需要测试PID控制器，可以取消下面这行的注释
			// Test_PID_Speed_Control();
		}
...
/**
  ...
  * @param  无
  * @retval 无
  * @note   此函数仅用于测试PID控制器功能
  */
void Test_PID_Speed_Control(void)
{
    // 测试PID速度控制
    printf("Testing PID speed control...\r\n");
    
    // 设置目标速度为100mm/s
    Speed_Control_Update(100.0f, 100.0f);
    HAL_Delay(2000);  // 持续2秒
    
    // 设置目标速度为200mm/s
    Speed_Control_Update(200.0f, 200.0f);
    HAL_Delay(2000);  // 持续2秒
    
    // 停止
    Car_Stop();
    printf("PID speed control test completed.\r\n");
}
