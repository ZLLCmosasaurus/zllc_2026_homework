...
void UART_Process_Command(void)
{
	...
	{
		char *left_speed_str = strtok(NULL, ",");
		char *right_speed_str = strtok(NULL, ",");
		
		if(left_speed_str == NULL || right_speed_str == NULL)
		{
			printf("ERROR: Invalid SPEED command\r\n");
			return;
		}
		
		float left_speed = atof(left_speed_str);
		float right_speed = atof(right_speed_str);
		
		// 使用PID控制器控制速度
		Speed_Control_Update(left_speed, right_speed);
		printf("OK: Speed control - Left: %.1f mm/s, Right: %.1f mm/s\r\n", left_speed, right_speed);
	}
	// 处理停止命令
...
/**
  ...
  *   CMD=0x06 SPEED: [left_speed:int32 LE][right_speed:int32 LE] (单位: mm/s)
  */
void UART_Process_Command_Array(const uint8_t* frame, uint16_t len)
{
	...
	switch(cmd)
	{
		...
		case 0x06: // SPEED control
		{
			if(N != 9) { printf("ERROR: SPEED payload\r\n"); break; }
			// 解析左轮速度 (int32 LE)
			int32_t left_speed_int = (int32_t)(payload[0] | (payload[1] << 8) | (payload[2] << 16) | (payload[3] << 24));
			// 解析右轮速度 (int32 LE)
			int32_t right_speed_int = (int32_t)(payload[4] | (payload[5] << 8) | (payload[6] << 16) | (payload[7] << 24));
			// 转换为浮点数速度 (mm/s)
			float left_speed = (float)left_speed_int / 10.0f;  // 假设速度以0.1 mm/s为单位存储
			float right_speed = (float)right_speed_int / 10.0f;
			// 使用PID控制器控制速度
			Speed_Control_Update(left_speed, right_speed);
			printf("OK: SPEED L:%.1f R:%.1f\r\n", left_speed, right_speed);
			break;
		}
		default:
