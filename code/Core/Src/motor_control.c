...
// PID控制器全局变量
static PID_Controller left_pid;
static PID_Controller right_pid;
...
void Speed_Control_Init(void)
{
    ...
    // 参数需要根据实际系统调试调整
    // 这里使用更合理的PID参数
    PID_Init(&left_pid, 1.5f, 0.8f, 0.05f, -MAX_PWM, MAX_PWM);
    PID_Init(&right_pid, 1.5f, 0.8f, 0.05f, -MAX_PWM, MAX_PWM);
}

/**
  * @brief  更新速度控制
  * @param  left_target_speed: 左轮目标速度(mm/s)
  * @param  right_target_speed: 右轮目标速度(mm/s)
  * @retval 无
  */
void Speed_Control_Update(float left_target_speed, float right_target_speed)
{
    // 设置目标速度
    PID_SetSetpoint(&left_pid, left_target_speed);
    PID_SetSetpoint(&right_pid, right_target_speed);
    
    // 获取当前实际速度
    float left_actual_speed = Speed_GetLeft();
    float right_actual_speed = Speed_GetRight();
    
    // PID计算得到PWM输出
    float left_pwm_output = PID_Compute(&left_pid, left_actual_speed);
    float right_pwm_output = PID_Compute(&right_pid, right_actual_speed);
    
    // 应用PWM输出到电机
    int16_t left_pwm = (int16_t)left_pwm_output;
    int16_t right_pwm = (int16_t)right_pwm_output;
    ...
    if (left_pwm > MIN_PWM) {
        ...
        LF_FORWARD();
        LB_FORWARD();
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, left_pwm);  // LF
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_2, left_pwm);  // LB
        ...
        LF_STOP();
        LB_STOP();
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, 0);  // LF
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_2, 0);  // LB
    }
    ...
    if (right_pwm > MIN_PWM) {
        ...
        RF_FORWARD();
        RB_FORWARD();
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_3, right_pwm);  // RF
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_4, right_pwm);  // RB
        ...
        RF_STOP();
        RB_STOP();
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_3, 0);  // RF
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_4, 0);  // RB
    }
}
